            ...
            <div class="accordion accordion-flush" id="servizi1">
                 @foreach ($leftServices as $service)
                    <div class="accordion-item">
                        <h2 class="accordion-header" id="implantologia-{{ $service->id }}">
                            <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#flush-collapseservizi1-{{ $service->id }}" aria-expanded="false" aria-controls="flush-collapseservizi1-{{ $service->id }}">
                            <h4 class="h4Accordion">{{ $service->name }}</h4>
                            </button>
                        </h2>
                        <div id="flush-collapseservizi1-{{ $service->id }}" class="accordion-collapse collapse" aria-labelledby="servizi1-{{ $service->id }}" data-bs-parent="#servizi1">
        ...
        <div class="col-12 col-md-5">
            <div class="accordion accordion-flush" id="servizi2">
                @foreach ($rightServices as $service)
                    <div class="accordion-item">
                        <h2 class="accordion-header" id="servizi2-{{ $service->id }}">
                            <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#flush-collapseservizi2-{{ $service->id }}" aria-expanded="false" aria-controls="flush-collapseservizi2-{{ $service->id }}">
                            <h4 class="h4Accordion">{{ $service->name }}</h4>
                            </button>
                        </h2>
                        <div id="flush-collapseservizi2-{{ $service->id }}" class="accordion-collapse collapse" aria-labelledby="servizi2-{{ $service->id }}" data-bs-parent="#servizi2">
                        <div class="accordion-body"><p>{{ $service->shortDescription }}</p></div>
                        </div>
                    </div>
                @endforeach
